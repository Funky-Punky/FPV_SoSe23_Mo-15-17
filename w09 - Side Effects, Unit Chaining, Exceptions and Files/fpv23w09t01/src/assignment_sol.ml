      ...
      | _ ->
          raise
            (My_Corrupt_database_file
               "A Grade line has the wrong number of fields")
    with
    | End_of_file ->
        raise
          (My_Corrupt_database_file
             "GradeCount for student was to large")
    | Failure _ ->
        raise
          (My_Corrupt_database_file
             "Argument in GradeLine couldn't be converted from \
              string")
...
let read_student_from_chan chan =
  try
    ...
    | _ ->
        raise
          (My_Corrupt_database_file
             "student line had the wrong number of fields")
  with
  | End_of_file -> None
  | Failure _ ->
      raise
        (My_Corrupt_database_file
           "Argument in Student line couldn't be converted from \
            string")
